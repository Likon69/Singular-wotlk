    ...
    public class Elemental
    {
        ...
        public static Composite CreateElementalShamanPreCombatBuffs()
        {
            return new PrioritySelector(
                new Decorator(
                    ret => !StyxWoW.Me.Inventory.Equipped.MainHand.TemporaryEnchantment.IsValid && StyxWoW.Me.Inventory.Equipped.MainHand.ItemInfo.WeaponClass != WoWItemWeaponClass.FishingPole && SpellManager.HasSpell("Flametongue Weapon") &&
        ...
        public static Composite CreateElementalShamanNormalCombat()
        {
            return new PrioritySelector(
                ...
                // WotLK QC: Removed Fulmination 7-stack check (Cata 4.0.1). WotLK Lightning Shield max 3 charges.
                // Earth Shock is used when Flame Shock has >6s remaining (no point refreshing FS)
        ...
        public static Composite CreateElementalShamanPvPPullAndCombat()
        {
            return new PrioritySelector(
                ...
                Spell.Buff("Flame Shock", true),
                Spell.Cast("Lava Burst"),
                // WotLK QC: Removed Fulmination 7-stack check (Cata 4.0.1)
                Spell.Cast("Earth Shock",
                    ret => StyxWoW.Me.CurrentTarget.GetAuraTimeLeft("Flame Shock", true).TotalSeconds > 6),
                Spell.Cast("Chain Lightning", ret => Unit.UnfriendlyUnitsNearTarget(10f).Count() >= 2),
                Spell.Cast("Lightning Bolt"),
        ...
        public static Composite CreateElementalShamanInstancePullAndCombat()
        {
            return new PrioritySelector(
                ...
                Spell.BuffSelf("Lightning Shield"),
                Spell.BuffSelf("Elemental Mastery",
                    ret => StyxWoW.Me.HasAnyAura("Bloodlust", "Heroism")),
                Spell.BuffSelf("Elemental Mastery", 
                    ret => StyxWoW.Me.IsMoving && StyxWoW.Me.Combat),
