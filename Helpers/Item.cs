...
namespace Singular.Helpers
{
    internal static class Item
    {
        ...
        public static bool HasWeaponImbue(WoWInventorySlot slot, string imbueName, int imbueId)
        {
            ...
            if (enchant != null)
            {
                Logger.Write("EnchantName: " + enchant.Name );
                Logger.Write("EnchantID: " + enchant.Id);
        ...
        public static Composite CreateUseAlchemyBuffsBehavior()
        {
            return new PrioritySelector(
                new Decorator(
                    ...
                    new PrioritySelector(
                        ctx => StyxWoW.Me.CarriedItems.FirstOrDefault(i => i.Entry == 47499),
                // Flask of the North (WotLK alchemy) — Flask of Enhancement (58149) is Cata-only, removed
                        new Decorator(
                            ret => ret != null,
                            new Sequence(
                                new Action(ret => Logger.Write(String.Format("Using {0}", ((WoWItem)ret).Name))),
