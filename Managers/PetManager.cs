    ...
    internal class PetManager
    {
        ...
        static PetManager()
        {
            ...
            Mount.OnDismount += (s, e) =>
                    {
                        // Check if player has a pet class (Hunter/Warlock) or has an active pet
                        if (StyxWoW.Me.Class == WoWClass.Hunter || StyxWoW.Me.Class == WoWClass.Warlock || StyxWoW.Me.HasPet)
                        {
                            PetTimer.Reset();
                        }
                    };
        ...
        public static bool CallPet(string petName)
        {
            ...
            switch (StyxWoW.Me.Class)
            {
                ...
                case WoWClass.Hunter:
                    // WotLK: Only one "Call Pet" spell exists (no numbered variants like Cata's "Call Pet 1/2/3")
                    if (SpellManager.CanCast("Call Pet"))
                    {
                        if (!StyxWoW.Me.GotAlivePet)
                        {
                            Logger.Write("[Pet] Calling out pet");
                            bool result = SpellManager.Cast("Call Pet");
                            //if (result)
                            //    StyxWoW.SleepForLagDuration();
                            return result;
                        }
